    ...
    <div v-else class="pm-grid">
      ...
      >
        <div class="pm-card-cover" @click="openEditModal(tpl)">
          <img
            v-if="tpl.cover"
            :src="resolveUrl(tpl.cover)"
            class="pm-cover-img"
            alt="cover"
          />
          <div v-else class="pm-cover-placeholder">📄</div>
          <span class="pm-card-badge" :class="{ vip: tpl.isVip }">
            {{ tpl.isVip ? 'VIP' : '免费' }}
          </span>
        </div>
        <div class="pm-card-body">
          <div class="pm-card-name" :title="tpl.name">{{ tpl.name }}</div>
          <div class="pm-card-cat">{{ getCategoryName(tpl.category) }}</div>
        ...
        <div class="pm-modal">
          ...
          <div class="pm-modal-body">
            ...
            <div class="pm-form-section">
              ...
              <div class="pm-form-row">
                <label>分类 *</label>
                <select v-model="form.category" class="pm-input">
              ...
              <div class="pm-form-row two-col">
                <div>
                  <label>封面图 URL</label>
                  <input v-model="form.cover" class="pm-input" placeholder="https://…" />
                </div>
                <div>
                  <label>背景图 URL</label>
                  <input v-model="form.backgroundImage" class="pm-input" placeholder="https://…" />
                </div>
              </div>
              <div class="pm-form-row">
                <label class="pm-toggle-label">
                  <span>{{ form.isVip ? '👑 VIP 模板' : '🆓 免费模板' }}</span>
                  <label class="pm-switch">
                    <input type="checkbox" v-model="form.isVip" />
            ...
            <div class="pm-form-section">
              <div class="pm-section-header">
                <span class="pm-section-title">可编辑区域</span>
              ...
              <div class="pm-canvas-wrap">
                <div class="pm-canvas" ref="canvasWrapRef">
                  <img
                    ...
                    :src="resolveUrl(form.backgroundImage)"
                    class="pm-canvas-bg"
                    alt="bg"
                  />
                  <div
              ...
              <div v-if="selectedAreaIdx !== null && form.editableAreas[selectedAreaIdx]" class="pm-area-props">
                ...
                <div class="pm-form-row two-col">
                  <div>
                    <label>X (%)</label>
                    <input type="number" min="0" max="100" step="1" v-model.number="form.editableAreas[selectedAreaIdx].x" class="pm-input" />
                  </div>
                  <div>
                    <label>Y (%)</label>
                    <input type="number" min="0" max="100" step="1" v-model.number="form.editableAreas[selectedAreaIdx].y" class="pm-input" />
                  </div>
                </div>
                <div class="pm-form-row two-col">
                  <div>
                    <label>宽 (%)</label>
                    <input type="number" min="1" max="100" step="1" v-model.number="form.editableAreas[selectedAreaIdx].width" class="pm-input" />
                  </div>
                  <div>
                    <label>高 (%)</label>
                    <input type="number" min="1" max="100" step="1" v-model.number="form.editableAreas[selectedAreaIdx].height" class="pm-input" />
                  </div>
                </div>
                <div class="pm-form-row" v-if="form.editableAreas[selectedAreaIdx].type === 'text'">
                  <label>字号 (px)</label>
                ...
                <div class="pm-form-row two-col" v-if="form.editableAreas[selectedAreaIdx].type === 'text'">
                  ...
                  <div>
                    <label>对齐</label>
                    <select v-model="form.editableAreas[selectedAreaIdx].textAlign" class="pm-input">
...
const defaultForm = (): PosterTemplate => ({
  name: '',
  category: '',
  cover: '',
  backgroundImage: '',
...
const form = reactive<PosterTemplate>(defaultForm())
...
function areaStyle(area: EditableArea): Record<string, string> {
  return {
    ...
    width: `${area.width}%`,
    height: `${area.height}%`,
    fontSize: area.fontSize ? `${area.fontSize * 0.4}px` : '12px',
    color: area.color || '#333',
    textAlign: area.textAlign || 'center',
  }
}
...
async function fetchStats() {
  try {
    ...
    if (res.data.success !== false && res.data.data) {
      ...
      stats.categoryCount = res.data.data.categoryCount ?? categories.length
      stats.freeCount = res.data.data.freeCount ?? 0
      stats.vipCount = res.data.data.vipCount ?? 0
    }
  } catch (_) {
    // API 不存在时用本地数据计算
    computeLocalStats()
...
function computeLocalStats() {
  ...
  stats.freeCount = list.filter(t => !t.isVip).length
  stats.vipCount = list.filter(t => t.isVip).length
}

function normalizeTemplate(raw: any): PosterTemplate {
  return {
    id: raw.id || raw._id,
    name: raw.name || '',
    ...
    isVip: raw.isVip ?? raw.is_vip ?? false,
    editableAreas: (raw.editableAreas || raw.editable_areas || []).map((a: any) => ({
      type: a.type || 'text',
      label: a.label || '',
      defaultText: a.defaultText || a.default_text || '',
      x: a.x ?? 0,
      y: a.y ?? 0,
      width: a.width ?? 30,
      height: a.height ?? 10,
...
async function onDeleteTemplate(tpl: PosterTemplate) {
...
function openEditModal(tpl: PosterTemplate) {
...
function addArea(type: 'text' | 'image') {
  const area: EditableArea = {
    ...
    height: 12,
    ...(type === 'text'
      ? { defaultText: '示例文字', fontSize: 16, color: '#333333', textAlign: 'center' as const }
      : {}),
  }
...
function onDragMove(e: MouseEvent) {
  ...
  area.x = Math.max(0, Math.min(100 - area.width, dragState.origX + dx))
  area.y = Math.max(0, Math.min(100 - area.height, dragState.origY + dy))
}
...
.pm-form-section + .pm-form-section {
  padding-top: 20px;
...
.pm-switch input:checked + .pm-slider { background: #ffd54f; }
...
.pm-area.selected {
  ...
  box-shadow: 0 0 0 2px rgba(232, 74, 110, 0.3);
}
...
.pm-area-del {
  ...
  height: 18px;
  border: none;
  background: rgba(232, 74, 110, 0.8);
  color: #fff;
  font-size: 12px;
  line-height: 1;
  border-radius: 50%;
...
.pm-area:hover .pm-area-del,
.pm-area.selected .pm-area-del { opacity: 1; }
