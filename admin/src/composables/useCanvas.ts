...
export function useCanvas(opts: UseCanvasOptions) {
  ...
  function addText(partial?: Partial<TextElement>) {
    ...
    const text = new fabric.IText(el.content, {
      ...
      // PS horizontalScale 文字横向压缩（如 0.876）：用 scaleX 围绕中心还原，
      // originX/Y=center，与导入布局（left+width/2）一致
      scaleX: el.horizontalScale && el.horizontalScale !== 1 ? el.horizontalScale : 1,
