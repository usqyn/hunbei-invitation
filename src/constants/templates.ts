import type { TemplateCategory, TemplateItem, TemplateData, EditableElement, ElementStyle, TemplateSettings, BasicInfo } from '@/types'
 ...
 *  一个完整的模板由 3 部分组成：
 *
 *  1. TEMPLATE_DATA - 模板的内容数据
 *     {
 *       coverImage:    string   // 封面图片路径
 *       coverTitle:    string   // 封面大标题
 *       coverSubtitle: string   // 封面英文副标题
 *       photo1:        string   // 第一张配图
 *       photo2:        string   // 第二张配图
 *       photo3:        string   // 第三张配图
 *       photo4:        string   // 第四张配图
 *       photoTitle:    string   // 内容区大标题
 *       photoSubtitle: string   // 内容区英文副标题
 *       footerText:    string   // 正文内容（支持换行 \n）
 *       footerSubText: string   // 底部标签文字
 *     }
 *
 *  2. TEMPLATE_ELEMENTS - 可编辑元素列表
 *     每个元素包含：
 *     {
 *       type:    'image' | 'text'  // 元素类型
 *       text:    string            // 显示内容（图片为路径）
 *       dataKey: keyof TemplateData // 关联到 data 中的字段
 *       label:   string            // 右侧编辑面板的提示文字
 *       style:   ElementStyle      // 文字元素的样式（仅 text 类型需要）
 *     }
 *     元素顺序 = 预览区从上到下的渲染顺序
 *
 *  3. TEMPLATE_ITEM - 模板的注册信息
 *     {
 *       id:           string      // 唯一ID，建议：{category}-{序号}，如 'wedding-3'
 *       name:         string      // 模板名称，如 '简约中式'
 *       subtitle:     string      // 副标题/描述文案
 *       category:     string      // 分类ID: wedding | birthday | baby | graduation | festival | business
 *       cover:        string      // 模板卡片封面图路径
 *       likes:        number      // 显示的喜欢数量
 *       pageCount:    number      // 显示的页数
 *       data:         TemplateData    // 上面的 TEMPLATE_DATA
 *       elements:     EditableElement[] // 上面的 TEMPLATE_ELEMENTS
 *       primaryColor: string      // 主题色（按钮/标签的颜色）
 *     }
 *
 *  添加步骤：
 *  ① 准备一张 SVG 封面图，放到 /src/static/images/templates/ 目录
 *  ② 在本文件顶部下方定义一个 XXX_DATA 和 XXX_ELEMENTS
 *  ③ 在 TEMPLATE_LIST 数组中添加一个 TemplateItem
 *  ④ 完成！CATEGORY_LIST 会自动根据 category 字段过滤，无需手动修改
 *
 * ============================================================
 */
...
export const FONT_LIST = ['思源宋体', '思源宋体极细', '思源黑体', '华文楷体', '华文行楷', '华文隶书']
...
// 素材图片库（右侧编辑面板的"素材"标签页展示）
export const MATERIAL_IMAGES = [
  '/static/images/templates/wedding-1.svg',
  '/static/images/templates/wedding-2.svg',
  '/static/images/templates/wedding-3.svg',
  '/static/images/templates/wedding-4.svg',
  '/static/images/templates/wedding-5.svg',
  '/static/images/templates/wedding-6.svg',
  '/static/images/templates/invitation-1.svg',
  '/static/images/templates/invitation-2.svg',
  '/static/images/templates/invitation-3.svg',
  '/static/images/templates/invitation-4.svg',
  '/static/images/templates/moments-1.svg',
  '/static/images/templates/moments-2.svg',
  '/static/images/templates/template-1.svg',
  '/static/images/templates/template-2.svg',
  '/static/images/templates/template-3.svg',
  '/static/images/templates/template-4.svg',
  '/static/images/templates/template-5.svg',
]
...
const BIRTHDAY_1_ELEMENTS: EditableElement[] = [
  ...
  { type: 'text', text: '🎂', label: '蛋糕图标', style: { ...DEFAULT_ELEMENT_STYLE, fontSize: 56 } },
...
const BIRTHDAY_2_ELEMENTS: EditableElement[] = [
  ...
  { type: 'text', text: '🎂🎉🎈', label: '派对图标', style: { ...DEFAULT_ELEMENT_STYLE, fontSize: 48 } },
...
const BABY_1_ELEMENTS: EditableElement[] = [
  ...
  { type: 'text', text: '👶', label: '宝宝图标', style: { ...DEFAULT_ELEMENT_STYLE, fontSize: 56 } },
...
const BABY_2_ELEMENTS: EditableElement[] = [
  ...
  { type: 'text', text: '🧸💝', label: '装饰图标', style: { ...DEFAULT_ELEMENT_STYLE, fontSize: 48 } },
...
const FESTIVAL_1_ELEMENTS: EditableElement[] = [
  ...
  { type: 'text', text: '🎊', label: '庆祝图标', style: { ...DEFAULT_ELEMENT_STYLE, fontSize: 56 } },
...
const FESTIVAL_2_ELEMENTS: EditableElement[] = [
  ...
  { type: 'text', text: '🌕🥮', label: '节日图标', style: { ...DEFAULT_ELEMENT_STYLE, fontSize: 48 } },
...
const BUSINESS_1_ELEMENTS: EditableElement[] = [
  ...
  { type: 'text', text: '🏢', label: '公司图标', style: { ...DEFAULT_ELEMENT_STYLE, fontSize: 56 } },
...
const BUSINESS_2_ELEMENTS: EditableElement[] = [
  ...
  { type: 'text', text: '🚀', label: '装饰图标', style: { ...DEFAULT_ELEMENT_STYLE, fontSize: 56 } },
...
// ============ 分类列表（自动根据 TEMPLATE_LIST 中的 category 字段过滤） ============
